...
class TestParent(TestParentCommon):
    ...
    def test_case_1_parent(self):
        ...
        vals = {
            'name': self.env.ref('gssc_core.op_res_partner_31').id,
            'user_id': self.env.ref('gssc_parent.user_parent').id,
            'relationship_id': self.env.ref(
                'gssc_parent.op_parent_relationship_1').id,
            'mobile': 8334845,
        }
        new_parent = self.op_parent.create(vals)
        new_parent.create_parent_user()
        student = self.env.ref('gssc_parent.user_parent').id
        val = {'mobile': 77777777}
    ...
    def test_case_2_student(self):
        vals = {
            'user_id': self.env.ref('gssc_parent.user_parent').id,
            'partner_id': self.env.ref('gssc_parent.res_partner_33').id,
            'name': 'nikul',
            'last_name': 'ahir',
            'gender': 'm',
            ...
            'mobile': '73482383624',
            'email': 'nik@gmail.com',
            'parent_ids': [(6, 0, [self.env.ref('gssc_parent.op_parent_1').id])],
        }
    ...
    def test_case_3_subject_registartion(self):
        vals = {
            ...
            'course_id': self.env.ref('gssc_core.op_course_2').id,
