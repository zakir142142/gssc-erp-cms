...
class TestAssignmentSubline(TestAssignmentCommon):
    ...
    def test_case_assignment_subline(self):
        ...
        assignment = self.env["op.assignment"].create({
            ...
            'marks': 50,
            'assignment_type':
                self.env.ref('gssc_assignment.op_assignment_1').id,
            'issued_date': time.strftime('%Y-%m-01'),
            'course_id': self.env.ref('gssc_core.op_course_4').id,
            'batch_id': self.env.ref('gssc_core.op_batch_3').id,
            'subject_id': self.env.ref('gssc_core.op_subject_10').id,
            'faculty_id': self.env.ref('gssc_core.op_faculty_2').id,
            'submission_date': time.strftime('%Y-%m-01'),
            'allocation_ids': self.env.ref("gssc_core.op_student_9"),
        ...
        assignment_subline1 = self.op_assignment_subline.create({
            ...
            'state': "draft",
            'student_id': self.env.ref("gssc_core.op_student_9").id,
